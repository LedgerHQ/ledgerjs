...
const { foreach, doIf, asyncWhile, splitPath, eachSeries } = require('./utils')
...
module.exports = class Btc {
  ...
  getWalletPublicKeyPrivate (path, verify, segwit) {
   ...
   */
  getWalletPublicKey (path, verify = false, segwit = false) {
    return this.getWalletPublicKeyPrivate(path, verify, segwit)
  }

  async getTrustedInputRaw (transactionData, indexLookup) {
    let data
  ...
  async getTrustedInput (indexLookup, transaction, additionals = []) {
  ...
  async provideOutputFullChangePath (path) {
  ...
  async hashOutputFull (outputScript, additionals = []) {
    let offset = 0
    let p1 = 0x80
    const isDecred = additionals.includes('decred')
      ...
      .then(() =>
        doIf(!resuming, () =>
          ...
          foreach(inputs, (input, i) =>
            this.getWalletPublicKeyPrivate(associatedKeysets[i], false, false)
          ).then(result => {
