...
async function impl(variant: "old" | "new", transport: Transport): Promise<Btc | BtcNew> {
  if (variant === "old") {
...
async function runGetWalletPublicKey(variant: "old" | "new") {
  ...
  try {
    ...
    const uncompressesPubkey = "04e84c7f4b7662faed9f5eb2d812d9b7bcf0e0bf2a33b17ac45e38b8459669c321816e33dd5541f01ac4e9dbee1ad25fccf2bdc1dc16336b3ae66463c093d612ed";
    expect(compressPublicKey(Buffer.from(uncompressesPubkey, 'hex'))).toEqual(expectedAccount.pubkey);
  
    expect(account.chainCode).toEqual(expectedAccount.chaincode.toString('hex'));
    expect(account.publicKey).toEqual(uncompressesPubkey);
    
    const keydata = await btc.getWalletPublicKey("m/44'/1'/0'/0/0");
    expect(keydata.bitcoinAddress).toEqual("mz5vLWdM1wHVGSmXUkhKVvZbJ2g4epMXSm");
  } finally {
    tr.close();
  }
}