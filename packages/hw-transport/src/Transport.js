...
TransportFoo.open(descriptor).then(transport => ...)
  ...
  setDebugMode (debug) {
    if (typeof debug === 'function') {
      ...
      this.debug = (msg) => console.log(msg)
    } else {
      this.debug = null
    }
  }
   ...
   */
  async send (cla, ins, p1, p2, data = Buffer.alloc(0), statusList = [StatusCodes.OK]) {
    if (data.length >= 256) {
...
TransportFoo.create().then(transport => ...)
   */
  static async create (openTimeout = 3000, listenTimeout = 10000) {
    return new Promise((resolve, reject) => {
      ...
      const listenTimeoutId = setTimeout(() => {
        sub.unsubscribe()
        reject(
          new TransportError(this.ErrorMessageListenTimeout, 'ListenTimeout')
        )
      }, listenTimeout)
    })
  }
  ...
  static ErrorMessageListenTimeout = 'No Ledger device found (timeout)'
  static ErrorMessageNoDeviceFound = 'No Ledger device found'
}
