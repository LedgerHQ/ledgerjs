...
const Transport = require('@ExodusMovement/hw-transport')
const { TransportError } = Transport
const getDevices = require('./getDevices')
const listenDevices = require('./listenDevices')
...
function defer () {
  ...
  // if (!_resolve || !_reject) throw new Error('defer() error') // this never happens and is just to make flow happy
 ...
 */
module.exports = class TransportNodeHid extends Transport {
  // device: HID.HID
  // ledgerTransport: boolean
  // timeout: number
  // exchangeStack: Array<*>

  constructor (device, ledgerTransport = true, timeout = 0) {
    super()
    this.device = device
  ...
  static setListenDevicesDebug = (debug) => {
    listenDevicesDebug =
