...
const devices = {
  ...
  nanoS: {
    ...
    getBlockSize: (firmwareVersion: string): number =>
      semver.lt(semver.coerce(firmwareVersion) as semver.SemVer, "2.0.0")
        ? 4 * 1024
        : 2 * 1024,
  },
  nanoX: {
 ...
 *
 */
export type DeviceModelId = keyof typeof devices;
...
export type DeviceModel = {
  ...
  getBlockSize: (_firmwareVersion: string) => number;
  bluetoothSpec?: Array<{
    serviceUuid: string;
    writeUuid: string;
    notifyUuid: string;
